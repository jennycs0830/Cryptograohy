 ...
 *
 * 2021-03-22 by Sean, 0816146 ­³µú²»
 */

using namespace std;

string decDES(const string keyStr, const string txtStr);
...
int main() {
	...
	key = "12345678";
	enc = "\x0A\xE4\x99\x12\x91\xDF\xB5\x52";

	//	key = "06220499";
	//	enc = "\x84\x97\x74\x87\xda\xd9\xd2\xd1";
...
string decDES(const string keyStr, string txtStr) {
	bool keyBin[64], txtBin[64], tmpBin[64];
	bool subBin[16][48], CiDi[56];
...
//		int flag = !(round == 0 || round == 1 || round == 8 || round == 15);
		...
		/* Permuted Choice 2 */
		for (int i = 0; i < 48; i++)
			subBin[round][i] = CiDi[table_pc2[i] - 1];
	}
	...
	for (int round = 15; round >= 0; round--) {
		...
		const bool *fstl = F(oldR, subBin[round]);
